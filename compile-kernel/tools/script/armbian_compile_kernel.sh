...
# https://github.com/khusnan21/amlogic-s9xxx-armbian
#
# Description: Run on Armbian, Compile the kernel.
# Copyright (C) 2021~ https://www.kernel.org
# Copyright (C) 2021~ https://github.com/unifreq
# Copyright (C) 2021~ https://github.com/khusnan21/amlogic-s9xxx-armbian
#
...
custom_name="-khusnan21"
pkg_maintainer="khusnan21 <noreply@khusnan21.org>"
...
init_var() {
    ...
    while true; do
        ...
        -n | --customName)
            if [[ -n "${2}" ]]; then
                custom_name="${2// /}"
                [[ -n "${custom_name}" ]] || custom_name="-khusnan21"
...
compile_kernel() {
    ...
    # Build and install out-of-tree RTL8189ES module
    echo -e "${STEPS} Compiling out-of-tree RTL8189ES module..."
    (
        cd "${compile_path}"
        rm -rf rtl8189ES_linux
        export KSRC="${kernel_path}/${local_kernel_path}"
        git clone --depth 1 https://github.com/jwrdegoede/rtl8189ES_linux.git
        cd rtl8189ES_linux
        make ARCH="${SRC_ARCH}" CROSS_COMPILE="${CROSS_COMPILE}" KSRC="${KSRC}" modules -j${PROCESS}
        if [[ -f 8189es.ko ]]; then
            mod_dir="${output_path}/modules/lib/modules/${kernel_outname}/kernel/drivers/net/wireless/realtek/rtl8189es"
            mkdir -p "${mod_dir}"
            cp -f 8189es.ko "${mod_dir}/"
            echo -e "${SUCCESS} RTL8189ES module compiled and installed successfully."
            # Update modules.dep
            depmod -b "${output_path}/modules" -a "${kernel_outname}" 2>/dev/null || true
        else
            echo -e "${WARNING} RTL8189ES module compilation failed."
        fi
    )
